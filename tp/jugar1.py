...
import random, sys, time, json, threading, ventana_bienvenida, Fichas, Tablero, IA,layout_py
...
def juego(cargar=False):
	...
	if dificultad == "Dificil":
		...
		layout = layout_py.crear_layout(tablero, tiempos, jugador, dificultad,cambios,opcion)    
	else:
		layout = layout_py.crear_layout(tablero, tiempos, jugador, dificultad,cambios)    
	...
	while True:                    
		...
		if event in (None,'EXIT'):
			...
			if layout_py.terminar(puntos,tiempos,jugador,dificultad):
				break
		...
		if tiempos[0]==0:
			layout_py.terminar_por_otros(puntos,jugador,dificultad)
			break
	window.close()
